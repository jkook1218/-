...
- 데이터 설정:<br/><br/>저는 실험을 계획 할 때 데이터에 따른 차이를 알고 싶은 것은 아니지만 데이터셋에 따라 결과가 다를 수 있기 때문에 3가지 데이터셋에 대해 결과를 살펴보고자 하였습니다. 데이터셋 마다 일정 비율(학습 데이터셋 중 80%)을 랜덤으로 지우는 방법을 통해 레이블이 없는 데이터를 강제로 생성해냈고 이에 대한 수치는 다음과 같습니다.
...
셀프/안셀프
*
모델 개수(logistic Reg, NN, KNN, … sklearn에서 지원하는거나 강필성교수님 코드로 구현가능한거 찾아보기)
=
2 *모델개수 만큼 비교

그리고 각각의 셀프 트레이닝에서 3가지 케이스에서는 어떤 차이가 있을까요? 
모델 개수(logistic Reg, NN, KNN, … sklearn에서 지원하는거나 강필성교수님 코드로 구현가능한거 찾아보기)
*
3가지 셀프트레이닝
=
