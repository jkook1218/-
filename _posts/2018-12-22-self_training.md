...
- **데이터 설정**:<br/><br/>실험을 계획 할 때 데이터에 따른 차이를 알고 싶은 것은 아니지만 데이터셋에 따라 결과가 다를 수 있기 때문에 3가지 데이터셋에 대해 결과를 살펴보고자 하였습니다. 데이터셋 마다 일정 비율(학습 데이터셋 중 80%)을 랜덤으로 지우는 방법을 통해 레이블이 없는 데이터를 강제로 생성해냈고 이에 대한 데이터의 개수 및 비율은 다음과 같습니다.<br/>(데이터는 uci 에서 제공하는 데이터셋 사이트에서 받아 이용하였습니다. https://archive.ics.uci.edu/ml/datasets.html)
...
![TEST](/img/test_result.png)<br/><br/>
